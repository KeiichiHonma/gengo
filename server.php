...
function wsOnMessage($clientID, $json_message, $messageLength, $binary) {
    ...
    if($message['type'] == 'call'){
        ...
        if(strcasecmp($type,TYPE_LANG_BACK) != 0){
            ...
            $con->safeExit();
            $mes_arr = array('call'=>'call','cid'=>$cid,'user' =>$user[0]['col_given_name'], 'skype' =>$user[0]['col_skype_name'], 'lang' =>$lang, 'manager' =>$free_manager[0]['col_given_name'], 'mid' =>$free_manager[0]['_id']);
    ...
    if($message['type'] == 'busy'){
        $con->isCommand = TRUE;
        $mes_text = '';
        if( is_numeric($message['cid']) && is_numeric($message['mid']) && is_numeric($message['old_mid']) ){
            if(strcasecmp($message['mid'],$message['old_mid']) != 0){
                //予定の担当者以外
                $call_handle = new callHandle();
            ...
            $busy_arr = array('busy'=>'busy','cid'=>$message['cid'],'manager'=>$message['manager']);
