                ...
                if($con->pagepath != 'entry/input' && (strcasecmp($setting[$i]['name'],'magazine_mail') == 0 || strcasecmp($setting[$i]['name'],'notification_mail') == 0)){
                    foreach ($setting[$i]['value'] as $key => $value){
                        ...
                        if(strlen($_POST['mail']) == 0 && strcasecmp($key,RECEIVE_PC) == 0){
                            $labe_id = '';
                            $data .= '<input disabled="true" type="radio" id="'.$labe_id.'" name="'.$setting[$i]['name'].'" value="'.$key.'" class="'.$setting[$i]['class'].'" '.$checked.' /><label for="'.$labe_id.'">'.$value.'</label>&nbsp;';
                            $data .= '(<span class="error_alert">！</span><a href="'.CAURL.'/mypage/member/edit/mail/input">パソコンのメールアドレスを設定</a>する必要があります。)';
                            $head_flag = TRUE;
                        //携帯で受信
                        }elseif(strlen($_POST['mail_m']) == 0 && strcasecmp($key,RECEIVE_MOBILE) == 0){
                            $labe_id = '';
                            $data .= '<br /><input disabled="true" type="radio" id="'.$labe_id.'" name="'.$setting[$i]['name'].'" value="'.$key.'" class="'.$setting[$i]['class'].'" '.$checked.' /><label for="'.$labe_id.'">'.$value.'</label>&nbsp;';
                            $data .= '(<span class="error_alert">！</span><a href="'.CAURL.'/mypage/member/edit/mail/input">携帯のメールアドレスを設定</a>する必要があります。)';
                            $foot_flag = TRUE;
                        //パソコン,携帯で受信
                        }elseif((strlen($_POST['mail']) == 0 || strlen($_POST['mail_m']) == 0) && strcasecmp($key,RECEIVE_ALL) == 0){
                            $labe_id = '';
                            $data .= '<br /><input disabled="true" type="radio" id="'.$labe_id.'" name="'.$setting[$i]['name'].'" value="'.$key.'" class="'.$setting[$i]['class'].'" '.$checked.' /><label for="'.$labe_id.'">'.$value.'</label>&nbsp;';
                            $data .= '(<span class="error_alert">！</span><a href="'.CAURL.'/mypage/member/edit/mail/input">パソコン、携帯両方のメールアドレスを設定</a>する必要があります。)';
                        }else{
